...
Maps SDK). Four modules, an experiment: they are here to find out how much of a map engine you
actually have to write, and where the approach falls over, before any of it is called `map-engine`.

- **`:experiment-map-engine`** — the component. Which tiles are wanted, and where they go on screen.
- **`:experiment-map-tiles`** — what a map needs regardless of what a tile holds: a fetch queue and
  the stand-in rule.
- **`:experiment-map-tiles-raster`** — image tiles. Depends on nothing but Compose.
- **`:experiment-map-tiles-vector`** — MVT tiles: a decoder, a redefinable style, and labels. Also
...
`:experiment-map-tiles` that something is `TileQueue<T>`, which keeps a want-list, runs a few workers over it
...
Not the engine's call. `:experiment-map-tiles` offers the usual answer as plain, testable logic:
...
`shown` lives in `:experiment-map-tiles`, which is why that module knows nothing about either tile format.
...
:experiment-map-tiles-raster
  RasterSlot(tile: ImageBitmap, src, modifier)

:experiment-map-tiles-vector
...
```
:experiment-map-engine            commonMain only
...
:experiment-map-tiles             commonMain only
...
:experiment-map-tiles-raster      commonMain only
  RasterSlot.kt      blit the src fraction of an ImageBitmap

:experiment-map-tiles-vector      commonMain only
...
```bash
./gradlew :experiment-map-engine:desktopTest :experiment-map-tiles:desktopTest
./gradlew :experiment-map-engine:iosSimulatorArm64Test :experiment-map-tiles:iosSimulatorArm64Test
./gradlew :experiment-map-engine:testAndroidHostTest :experiment-map-tiles:testAndroidHostTest \
          :experiment-map-tiles-vector:testAndroidHostTest

# that the libraries really are multiplatform
./gradlew :experiment-map-tiles-vector:compileKotlinJs :experiment-map-tiles-vector:compileKotlinWasmJs \
          :experiment-map-tiles-raster:compileKotlinIosArm64
```
