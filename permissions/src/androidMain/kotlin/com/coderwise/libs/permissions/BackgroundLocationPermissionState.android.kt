...
actual fun rememberBackgroundLocationPermissionState(): BackgroundLocationPermissionState {
    ...
    var status by remember { mutableStateOf(resolveBackgroundLocationStatus(context, activity)) }

    LifecycleResumeEffect(context, activity) {
        status = resolveBackgroundLocationStatus(context, activity)
        onPauseOrDispose { }
    }
    ...
    val launcher = rememberLauncherForActivityResult(ActivityResultContracts.RequestPermission()) {
        val newStatus = resolveBackgroundLocationStatus(context, activity)
        status = newStatus
...
private fun resolveBackgroundLocationStatus(context: Context, activity: Activity?): PermissionStatus {
