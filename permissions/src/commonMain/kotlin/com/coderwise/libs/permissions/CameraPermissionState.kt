 ...
 * on iOS. On the web targets it is the Permissions API's `camera` state, and asking means
 * opening a media stream — there is no separate request. Desktop has no camera permission
 * model at all, so [status] is always [PermissionStatus.Granted] there.
 *
